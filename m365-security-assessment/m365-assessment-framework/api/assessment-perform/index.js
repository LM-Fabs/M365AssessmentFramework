...
functions_1.app.http('assessment-perform', {
 ...
 */
async function performAssessmentHandler(request, context) {
...
async function performSecurityAssessment(request, context) {
    ...
    try {
        ...
        const customer = await utils_1.dataService.getCustomer(requestData.customerId);
        if (!customer) {
            return {
                status: 404,
                headers: utils_1.corsHeaders,
        ...
        try {
            ...
            const assessment = await utils_1.dataService.createAssessment(fallbackAssessment);
            return {
                status: 201,
                headers: utils_1.corsHeaders,
                body: JSON.stringify({
 ...
 */
function calculateLicenseMetrics(licenseDetails) {
 ...
 */
function calculateSecureScoreMetrics(secureScore) {
    if (!secureScore) {
        return {
            percentage: 0,
            currentScore: 0,
    ...
    const controlScores = (secureScore.controlScores || []).map((control) => ({
 ...
 */
function calculateOverallScore(licenseMetrics, secureScoreMetrics) {
 ...
 */
function generateRecommendations(licenseMetrics, secureScoreMetrics, secureScore) {
    ...
    if (secureScore && secureScore.controlScores) {
        const lowScoreControls = secureScore.controlScores
            .filter((control) => (control.score || 0) < 50)
